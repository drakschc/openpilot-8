...
def below_engage_speed_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  return NoEntryAlert(f"請保持 {get_display_speed(CP.minEnableSpeed, metric)} 以上速度行駛以啟用輔助駕駛")


def below_steer_speed_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  return Alert(
    f" {get_display_speed(CP.minSteerSpeed, metric)} 以下速度行駛時無法自動轉向",
    "",
...
def out_of_space_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  return NormalPermanentAlert("存儲空間不足", f"{full_perc}% 已使用")
...
def comm_issue_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  return NoEntryAlert(msg, alert_text_1="進程通訊故障")
...
def overheat_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  ...
  return NormalPermanentAlert("系統運行過熱", f"{temp:.0f} °C")


def low_memory_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  return NormalPermanentAlert("內存不足", f"{sm['deviceState'].memoryUsagePercent}% 已使用")


def high_cpu_usage_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  x = max(sm['deviceState'].cpuUsagePercent, default=0.)
  return NormalPermanentAlert("CPU 佔用率過高", f"{x}% 已使用")
...
def invalid_lkas_setting_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  text = "切換原廠車道保持輔助系統（LKAS）的開關以啟用"
  if CP.brand == "tesla":
    text = "切換到 主動巡航控制（TACC） 以啟用"
  elif CP.brand == "mazda":
    text = "啟用車輛的LKAS系統以啟用"
  elif CP.brand == "nissan":
    text = "禁用原車的LKAS系統以啟用"
  return NormalPermanentAlert("LKAS設置無效", text)
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventName.startupNoCar: {
    ET.PERMANENT: StartupAlert("車輛未識別"),
  },
  ...
  EventName.invalidLkasSetting: {
    ...
    ET.NO_ENTRY: NoEntryAlert("車道保持輔助系統設置無效"),
  },
  ...
  EventName.fanMalfunction: {
    ET.PERMANENT: NormalPermanentAlert("散熱風扇故障", "這是硬件故障"),
  },
  ...
  EventName.cameraMalfunction: {
    ...
    ET.NO_ENTRY: NoEntryAlert("相機故障，請嘗試重啟設備"),
  },
  ...
  EventName.resumeBlocked: {
    ET.NO_ENTRY: NoEntryAlert("按下Set鍵以啟用"),
  },
  ...
  EventName.steerTempUnavailable: {
    ET.SOFT_DISABLE: soft_disable_alert("自動轉向不可用"),
    ET.NO_ENTRY: NoEntryAlert("自動轉向不可用"),
  },

  EventName.steerTimeLimit: {
    ET.SOFT_DISABLE: soft_disable_alert("轉向時間限制"),
    ET.NO_ENTRY: NoEntryAlert("轉向時間限制"),
  },

  EventName.outOfSpace: {
    ET.PERMANENT: out_of_space_alert,
    ET.NO_ENTRY: NoEntryAlert("存儲空間不足"),
  },
  ...
  EventName.espDisabled: {
    ET.SOFT_DISABLE: soft_disable_alert("電子穩定控制系統已禁用"),
    ET.NO_ENTRY: NoEntryAlert("電子穩定控制系統已禁用"),
  },
  ...
  EventName.commIssue: {
    ET.SOFT_DISABLE: soft_disable_alert("進程通信故障"),
  ...
  EventName.lowMemory: {
    ET.SOFT_DISABLE: soft_disable_alert("內存不足: 請重啟設備"),
    ET.PERMANENT: low_memory_alert,
    ET.NO_ENTRY: NoEntryAlert("內存不足: 請重啟設備"),
  },

  EventName.accFaulted: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert("巡航故障: 請嘗試重啟車輛"),
    ET.PERMANENT: NormalPermanentAlert("巡航故障: 請嘗試重啟車輛"),
    ET.NO_ENTRY: NoEntryAlert("巡航故障: 請嘗試重啟車輛"),
  },

  EventName.espActive: {
    ET.SOFT_DISABLE: soft_disable_alert("電子穩定系統已激活"),
    ET.NO_ENTRY: NoEntryAlert("電子穩定系統已激活"),
  },
  ...
  EventName.canError: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert("車型數據不匹配"),
    ET.PERMANENT: Alert(
      "車型數據不匹配",
      "",
      AlertStatus.normal, AlertSize.small,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, 1., creation_delay=1.),
    ET.NO_ENTRY: NoEntryAlert("車型數據不匹配"),
  },

  EventName.canBusMissing: {
    ET.IMMEDIATE_DISABLE: ImmediateDisableAlert("CAN Bus Disconnected"),
    ET.PERMANENT: Alert(
      "CAN Bus Disconnected: 請檢查連接",
      "",
      AlertStatus.normal, AlertSize.small,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, 1., creation_delay=1.),
    ET.NO_ENTRY: NoEntryAlert("CAN Bus Disconnected: 請檢查連接"),
  },
  ...
  EventName.speedTooHigh: {
    ...
    ET.NO_ENTRY: NoEntryAlert("請減速以恢復輔助駕駛"),
  },
